...
module.exports = {
  ...
  m_n_associations1: async (req, res) => {
    try {
      const {username, points} = req.body;
      const {name} = req.body;
      const {selfGranted} = req.body;
      ...
      await customerData.addProfile(profileData, { through: { selfGranted: selfGranted } });
  ...
  scopes: async (req,res) => {  // for reusibility purpose
    db.player.addScope('checkStatus',{
      where: {
        status: 0,
      }
    })
    try {
      const data = await db.player('chechStatus').findAll({});
      res.status(200).send({data});
    } catch (error) {
